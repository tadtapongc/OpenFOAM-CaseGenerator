...
### Fixed
- **Writing a case's scripts no longer needs the geometry resolved first.** `mesher.mesh_plan()` built every plan through `resolve_snappy_settings` / `resolve_cfmesh_settings`, so any config that had not been through `resolve_mesh_params(cfg, combined_bounds)` — a bare `DEFAULT_CONFIG`, or any caller using `write_scripts` on its own — died on `KeyError: 'min'` (snappy wanted a `domain_box`) or `ValueError: mesh_params.base_cell_size 'auto' must be resolved from the geometry first` (cfMesh wanted metres). Neither engine's plan actually reads a geometry-derived value: snappy takes its MPI policy from `parallel.n_procs`, cfMesh from `cfmesh.parallel_meshing` plus `cfmesh.feature_angle`. `snappy_plan_from_config` / `cfmesh_plan_from_config` now build the plan from those declared keys, and `snappy_mesh_plan` / `cfmesh_mesh_plan` delegate to the same pipeline so the resolved-settings path is unchanged. This reached `main` unnoticed because the only test that wrote scripts from an unresolved config, `tests/test_shell_scripts.py`, is `skipUnless(posix and bash)` and therefore never runs on the Windows CI jobs; `tests/test_scripts_from_bare_config.py` now covers both meshers on every platform and pins the config-only plan equal to the plan built from resolved settings.
